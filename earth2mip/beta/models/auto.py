...
from modulus.utils import _download_cached

from earth2mip.beta.models.dx import DiagnosticModel
from earth2mip.beta.models.px import PrognosticModel
...
class Package:
    ...
    cache_dir : Optional[str], optional
        ...
        If that is empty the path "~/.cache/earth2mip" is used, by default None
    """
    ...
    def __init__(self, root: str, cache_dir: Optional[str] = None):
        ...
        if not self.cache_dir:
            default_cache = os.path.join(os.path.expanduser("~"), ".cache", "earth2mip")
            self.cache_dir = os.environ.get("LOCAL_CACHE", default_cache)
    ...
    def get(self, path: str, recursive: bool = False) -> str:
        ...
        return _download_cached(
            self._fullpath(path), recursive=recursive, local_cache_path=self.cache_dir
        )
...
class AutoModel:
    ...
    def load_model(
        cls,
        package: Package,
    ) -> Union[PrognosticModel, DiagnosticModel]:
    ...
    def from_pretrained(
        ...
        """Loads and instantiates a pre-trained Earth-2 MIP model
